...
contract GetterFlow is Script {
    function run() external view {
        ...
        console.log("Protocol Token on Flow Spoke at:", spoke.checkProtocolToOriginal(1));
        assert(spoke.checkProtocolToOriginal(1) == flowUsdtToken);
        console.log("Protocol Token on Flow Pool at:", pool.checkProtocolToOriginal(1));
        assert(pool.checkProtocolToOriginal(1) == flowUsdtToken);
        ...
        // Check Hub Contract
        console.log("Hub Contract of Flow Spoke contract on Arbitrum Chain :", spoke.getHubAddress());
        assert(spoke.getHubAddress() == hubAddress);

        // Check Pool Addr
        console.log("Pool Address on Flow Spoke chain", spoke.getPoolAddr());
        ...
        console.log("Hub Chain Selector on Flow Spoke contract for Arbitrum Chain :", spoke.getHubChainSelector());
        ...
        console.log("USDT balance in the TokenPool:", IERC20(flowUsdtToken).balanceOf(poolAddr));

        // USDT Balance of Spoke on Flow
        console.log("USDT balance of Spoke on Flow:", IERC20(flowUsdtToken).balanceOf(spokeAddress));

        // USDT Balance of Receiver on Flow
        console.log("USDT balance of Receiver on Flow:", IERC20(flowUsdtToken).balanceOf(receiver));
    }
}

// forge script script/Spoke/Flow/Getter/GetterFlow.s.sol:GetterFlow --account defaultKey --sender $WALLET_ADDRESS --rpc-url $FLOW_TESTNET_RPC_URL -vvv
