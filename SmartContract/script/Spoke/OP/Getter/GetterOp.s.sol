...
contract GetterOp is Script {
    function run() external view {
        ...
        console.log("Protocol Token on OP Spoke at:", spoke.checkProtocolToOriginal(1));
        assert(spoke.checkProtocolToOriginal(1) == opUsdtToken);
        console.log("Protocol Token on OP Pool at:", pool.checkProtocolToOriginal(1));
        assert(pool.checkProtocolToOriginal(1) == opUsdtToken);
        ...
        // Check Hub Contract
        console.log("Hub Contract of OP Spoke contract on Arbitrum Chain :", spoke.getHubAddress());
        assert(spoke.getHubAddress() == hubAddress);

        // Check Pool Addr
        console.log("Pool Address on OP Spoke chain", spoke.getPoolAddr());
        ...
        console.log("Hub Chain Selector on OP Spoke contract for Arbitrum Chain :", spoke.getHubChainSelector());
        assert(spoke.getHubChainSelector() == hubChain);

        // Balance of Base TokenPool in TokenPool
        console.log("USDT balance on OP in the TokenPool:", IERC20(opUsdtToken).balanceOf(poolAddr));

        // Balance of Base TokenPool in Spoke
        console.log("USDT balance on OP in the Spoke:", IERC20(opUsdtToken).balanceOf(spokeAddress));

        // Balance of Base TokenPool in Spoke
        console.log("USDT balance of Owner on OP chain:", IERC20(opUsdtToken).balanceOf(owner));

        // Balance of Base TokenPool in Spoke
        console.log("USDT balance of Receiver on OP chain:", IERC20(opUsdtToken).balanceOf(receiver));
...
// forge script script/Spoke/OP/Getter/GetterOp.s.sol:GetterOp --account defaultKey --sender $WALLET_ADDRESS --rpc-url $OP_SEPOLIA_RPC_URL -vvv
