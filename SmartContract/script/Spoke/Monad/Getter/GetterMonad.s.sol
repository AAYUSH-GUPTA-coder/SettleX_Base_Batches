...
contract GetterMonad is Script {
    function run() external view {
        ...
        console.log("Protocol Token on Monad Spoke at:", spoke.checkProtocolToOriginal(1));
        assert(spoke.checkProtocolToOriginal(1) == monadUsdtToken);
        console.log("Protocol Token on Monad Pool at:", pool.checkProtocolToOriginal(1));
        assert(pool.checkProtocolToOriginal(1) == monadUsdtToken);
        ...
        console.log("USDT balance on Base in the TokenPool:", IERC20(monadUsdtToken).balanceOf(poolAddr));

        // Balance of Base TokenPool in Spoke
        console.log("USDT balance of Spoke on Base:", IERC20(monadUsdtToken).balanceOf(spokeAddress));

        console.log("USDT balance of Receiver on Base:", IERC20(monadUsdtToken).balanceOf(receiver));
    }
}

// forge script script/Spoke/Monad/Getter/GetterMonad.s.sol:GetterMonad --account defaultKey --sender $WALLET_ADDRESS --rpc-url $MONAD_SEPOLIA_RPC_URL -vvv
