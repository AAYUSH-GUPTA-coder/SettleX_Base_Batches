...
export const checkApprovalNeeded = async (
  ...
  try {
    ...
    const requiredAmount = parseUnits(amount.replace(/,/g, ""), 6);
      ...
      args: [
        {
          ...
          amount: parseUnits(amount.replace(/,/g, ""), 6),
        },
      ],
    });
...
export const handleApproval = async (
  ...
  const approvalAmount = parseUnits(amount.replace(/,/g, ""), 6);
...
export const handleGetBalance = async (
  ...
  try {
    ...
    // Convert from wei to token units (assuming 6 decimals)
